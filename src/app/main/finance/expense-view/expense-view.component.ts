 ...
 *****************************************************/
  getListrecords() {
    this.commonService
      .callApi('api/finance/expenses', this.data, 'get')
      .then((success) => {
        if (success) {
      ...
      .catch((e) => {
        console.log('there is an error1:', e);
      });
  }
  /*****************************************************
...
*****************************************************/
  deleteAdjustment(i: any) {
    this.swal({
      ...
      if (result.value) {
        this.commonService
          .callApi('api/finance/expenses/' + i.id, '', 'delete')
          .then((success) => {
            this.listrecords = [];
            this.data.offset = 0;
            this.getListrecords();
 ...
 *****************************************************/
  getexpenceCat() {
    this.commonService
      .callApi('api/tenants/expensecategories/lookup', '', 'get')
      .then((success) => {
        if (success) {
...
*****************************************************/
  ...
  createExpence(data: any) {
    if (data.id) {
      this.commonService
        .callApi('api/finance/expenses/' + data.id, this.user, 'put')
        .then((success) => {
          this.user = {};
          this.user['entryDate'] = new Date();
          if (success) {
      ...
      this.commonService
        .callApi('api/finance/expenses', this.user, 'post')
        .then((success) => {
          this.user = {};
          this.user['entryDate'] = new Date();
          this.user.isDebit = true;
