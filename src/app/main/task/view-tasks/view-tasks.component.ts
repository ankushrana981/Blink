...
throw new Error('Method not implemented.');
    ...
    constructor(
        ...
        // private mScrollbarService: MalihuScrollbarService, // private scrollToService: ScrollToService
   ...
   *****************************************************/
    ...
    onScrollDown(ev) {
        if (this.folderView) {
            ...
            if (boardviewcount > 0 && boardviewcount > this.limit && this.maxPage > 0 && this.page == 0 && boardviewcount != this.boardViewRecordsTotal) {
                this.page = (this.maxPage - (Math.ceil((this.boardViewRecordsTotal - boardviewcount) / (this.limit))));
            }
            ...
            if (listcount > 0 && listcount > this.limit && this.maxPage > 0 && this.page == 0 && listcount != this.listrecordsTotal) {
                this.page = (this.maxPage - (Math.ceil((this.listrecordsTotal - listcount) / (this.limit))));
        ...
        if (this.page < this.maxPage) {
            ...
            if (this.folderView) { this.getBoardViewData(); }
            else { this.getlistApi(); }
    ...
    createTaskModal(
        ...
        this.taskDetails = { taskData: taskData, subTaskData: editSubTaskData, listOfTask: this.listrecords };
  ...
  *****************************************************/
    ...
    *****************************************************/
    ...
    applyFilter(queryParams, terminate?) {
        ...
        if (this.folderView) {
            var queryParams2 =
                ...
                this.isCompletedTask;
            this.commonService
                .callApi("api/clients/tasks/folderview?" + queryParams2, queryParams, "get")
                .then((success) => {
                    if (success) {
                        success.records.forEach((obj) => {
                            obj.active = true;
    ...
    addTaskQuery() {
        if (this.addTaskQueryForm.valid) {
            ...
            this.commonService
                ...
                .then((success) => {
                    if (success) {
                        let getRecord = this.listrecords.find((x) => x.id == this.openPopupTaskId);
                        if (getRecord != undefined && getRecord != null) {
                            let index = this.listrecords.indexOf(getRecord);
                            let subTaskRec = this.listrecords[index].subTaskList.find((x) => x.id == this.openPopupSubTaskId);
                            if (subTaskRec != undefined && subTaskRec != null) {
                                let subTaskIndex = this.listrecords[index].subTaskList.indexOf(subTaskRec);
                                this.listrecords[index].subTaskList[subTaskIndex].showQuery = true;
                            }
                        }
                        this.modalRef.hide();
                    ...
                    .then((success) => {
                        if (success) {
                            ...
                            if (getRecord != undefined && getRecord != null) {
                                ...
                                if (subTaskRec != undefined && subTaskRec != null) {
                                    let subTaskIndex = this.listrecords[index].subTaskList.indexOf(subTaskRec);
                                    this.listrecords[index].subTaskList[subTaskIndex].showChat = true;
                                }
                            }
                ...
                .then((success) => {
                    if (success) {
                        ...
                        if (getRecord != undefined && getRecord != null) {
                            ...
                            if (subTaskRec != undefined && subTaskRec != null) {
                                let subTaskIndex = this.listrecords[index].subTaskList.indexOf(subTaskRec);
                                this.listrecords[index].subTaskList[subTaskIndex].showDetails = true;
                            }
                        }
    ...
    completeTask(mainTaskId) {
        if (mainTaskId > 0) {
            this.commonService.callApi("api/clients/tasks/" + mainTaskId + "/changestatus", "", "put")
                .then((success: any) => {
                    if (success) {
                        this.completeinfo = false;
                        let mainTaskRecord = this.folderView ? this.boardviewrecords.find((x) => x.id == mainTaskId) : this.listrecords.find((x) => x.id == mainTaskId);
                        if (mainTaskRecord) {
                            mainTaskRecord.isComplete = (!mainTaskRecord.isComplete) ? true : mainTaskRecord.isComplete;
                            mainTaskRecord.status = 2;
                        }
                    }
                    else { this.popToast("error", "Could not complete the task, Please try again later."); }
                })
                .catch((e) => { this.popToast("error", "Could not complete the task, Please try again later."); });
        }
        else { this.popToast("error", "Something went wrong while completing task"); }
    }
